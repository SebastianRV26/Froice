...
const RegisterConfirmation = () => {
    ...
    let [sendConfirmationEmail, loading, error] = useConfirmationEmail();
    const navigate = useNavigate();

    const reSendConfirmation = async() =>{
        setAuthData(await auth.currentUser);
    ...
    useEffect(() => {
        ...
        // I need this to run only at the start. Good bye for now warning
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [sendConfirmationEmail, auth]);
    */
    const verifyHandler = async () => {
        setAuthData(await auth.currentUser);
        await authData.reload();
